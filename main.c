...
static void _print_massive(int *__massive, int __size, char *__name)
{
	printf("%s: ", __name);
	for(int i = 0; i < __size; i++)
	{
		printf("%d ", __massive[i]);
	}
	printf("\n");
}
...
int main(void)
{
    ...
    printf("3");
    _print_massive(package_api.buffer + package_api.start_package_byte, package_api.stop_package_byte - package_api.start_package_byte, "buffer");
    return 1;
}
